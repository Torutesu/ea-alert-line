...
※本サービスは **LINE有料プラン前提**で運用する（速報込みの通数は受信者1人あたり月300〜500通の実測で、無料枠200通を超えるため）。ライトプラン（月5,000円/5,000通）で受信者10名程度まで。通知種別は `config.yaml` の `notices` で個別にオフにできる。
...
- **tickは外部要因のエラーでは落とさない**（落とすと状態DBがコミットされず、
  次のtickが同じ通知を作り直して再送＝通数を二重に消費するため）。
  ログに `LINE送信に失敗` / `statement一覧の取得に失敗` が出ていたら以下を確認:
  - LINEが **429** … 通数上限またはレート超過。LINE Official Account Manager の
    「メッセージ利用状況」で残通数を確認し、足りなければプラン変更か
    `config.yaml` の `notices` で通知種別を絞る（ログにAPIの応答本文が出るので
    「月間上限」か一時的なレート超過かはそこで判別できる）
  - みんかぶが **403/5xx** … 一時的なブロック。その回のポーリングだけスキップされ、
    次のtick（5分後）で自動復帰する
  - LINEが **401/403** … トークン不正・権限不足は設定ミスなのでジョブは失敗させる
    （Secrets の `LINE_CHANNEL_ACCESS_TOKEN` を再発行して登録し直す）
